...
(function () {
  ...
  // Every scene goes to 16x, in finer steps past 8x, where a wheel notch
  // that doubles the map throws the reader off the spot they were looking
  // at. A region's 10 km cell fills the view from 8x, so the base map
  // carries every village for those zooms and the dark of a silent cell
  // lets the roads under it ghost through: what is under the cell stays
  // readable, even if the reading itself is 10 km coarse.
  const zooms = () => [1, 2, 4, 8, 10, 12, 16];
